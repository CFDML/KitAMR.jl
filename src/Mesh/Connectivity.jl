...
function set_connectivity(global_data::Global_Data{DIM}) where {DIM}
    ...
    if periodic_dirs != ntuple(_ -> false, DIM)
        ...
        connectivity_ps = Cartesian_connectivity(
            global_data.config.trees_num...,
            global_data.config.geometry...,
        )
    end
...
function set_periodic_connectivity(global_data::Global_Data{DIM}, periodic_dirs) where {DIM}
    ...
    connectivity_ps = P4estTypes.brick(Tuple(global_data.config.trees_num), periodic_dirs)
    ...
    for i in eachindex(vertices)
        ...
        vertices[i] = ntuple(
            j -> begin
                if j <= DIM
                    normalized_coords[j] * (bounds[j][2] - bounds[j][1])/global_data.config.trees_num[j] +
                    bounds[j][1]
                else
                    0.0
                end
