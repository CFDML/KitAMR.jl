function pre_vs_refine!(
    ...
    ds = [
        ...
        global_data.config.vs_trees_num[i] for i = 1:DIM
    ]
                ...
                while index < vs_data.vs_num + 1
                    ...
                    if vs_data.level[index] < global_data.config.solver.AMR_VS_MAXLEVEL && (
                        ...
                        contribution_refine_flag(
                            ...
                            midpoint,
                            df,
                            vs_data.weight[index],
                            vr,
                            global_data,
                        ) :
                        vs_refine_udf(
                            midpoint;
                            level = vs_data.level[index],
                            du = ds ./ 2^vs_data.level[index],
                        )
                    )
                        midpoint_new =
                            midpoint_refine(DIM, midpoint, vs_data.level[index], ds)
...
$(SIGNATURES)
"""
function midpoint_refine_replace!(
    DIM::Integer,
    lnmidpoint::AbstractVector,
    midpoint_new::AbstractMatrix,
    vs_num::Int,
    index::Int,
)
    for i = 1:DIM
...
$(SIGNATURES)
"""
function df_refine_replace!(
    DIM::Integer,
    NDF::Integer,
    lndf::AbstractVector,
    df_new::AbstractMatrix,
    vs_num::Int,
    index::Int,
)
    for i = 1:NDF
...
$(SIGNATURES)
"""
df_refine(DIM::Integer, ::AbstractVector, ::AbstractMatrix, df::AbstractVector) =
    ones(2^DIM) * df'
"""
$(TYPEDSIGNATURES)
"""
    ...
    for i in eachindex(trees.data)
        for j in eachindex(trees.data[i])
            ...
            while index < vs_data.vs_num + 1
                ...
                if first_level > 0
                    if flag[first_level]%1==0.0 && all(
                        x -> x == first_level,
                        @view(vs_data.level[(index+1):(index+2^DIM-1)])
                    )
...
$(SIGNATURES)
"""
function df_coarsen_replace!(
    DIM::Integer,
    NDF::Integer,
    lndf::AbstractVector,
    df_new::AbstractVector,
    vs_num::Int,
    index::Int,
)
    for i = 1:NDF
...
function vs_resolution(ps_data::PS_Data{DIM,NDF}, global_data) where {DIM,NDF}
    ...
    weight =
        reduce(*, du)/reduce(*, vs_trees_num)/2^(
            DIM*(global_data.config.solver.AMR_VS_MAXLEVEL)
        )
