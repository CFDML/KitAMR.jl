...
function contribution_refine_flag(
    ...
    return local_contribution_refine_flag(
        w,
        U,
        midpoint,
        df,
        weight,
        global_data,
    )||global_contribution_refine_flag(U, midpoint, df, weight, vr, global_data)
end
"""
$(TYPEDSIGNATURES)
"""
function contribution_coarsen_flag(
    ...
    return local_contribution_coarsen_flag(
        w,
        U,
        midpoint,
        df,
        weight,
        global_data,
    )&&global_contribution_coarsen_flag(U, midpoint, df, weight, vr, global_data)
end
...
function global_contribution_coarsen_flag(
    ...
    df[1]*weight < ADAPT_COEFFI_VS*vr.density/2^DIM &&
        0.5 *
        (sum((U - midpoint) .^ 2) * df[1] + df[2]) *
        weight<vr.energy*ADAPT_COEFFI_VS/2^DIM
end
function global_contribution_coarsen_flag(
    U::AbstractVector,
    midpoint::AbstractVector,
...
function macro_estimate_IB_refine_flag(prim, bc, U, midpoint, ds, level)
    ...
    if (
        norm(
            ...
            ds,
        )/2^(level+1)<√(1/prim[end])||norm(midpoint-Ub)-norm(ds)/2^(level+1)<√(1/bc[end])
    )
        return true
    else
        return false
