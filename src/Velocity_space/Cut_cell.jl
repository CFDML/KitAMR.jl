...
function cut_rect(n::Vector{Float64}, vertices::Vector{Vector{Float64}}) # The splitting line at the boundary with normal direction n. Return the areas of the two part. The former one locates at the direction n.
    ...
    clp = findall(x->dot(x, n)==0.0, vertices) # Number of vertices lying on the discontinuity
    if length(clp)+i<2
        ...
        B = Matrix{Float64}(undef, 2, length(indices)-(bid-aid)+1)
        for i = aid:bid
            A[:, i-aid+1] .= points[i]
        end
        for i = 0:(length(indices)-(bid-aid))
...
function Vertical_Volume_Flux(
    ...
    for i in eachindex(points)
        ...
        if length(id)>1
            ...
            H+=sign(
                p1[dir]-midpoint[dir],
            )*p1[dir]/3.0*gaussian_area(@views A[[id1, id2], id_gauss])
        end
    end
    return H
end
