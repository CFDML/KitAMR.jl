    ...
    @inbounds for i = 1:n
        if refine_flags[i]
            ...
            for c = 1:nc
                ...
                for d = 1:DIM
                    dmid[w, d] =
                        vs_data.midpoint[i, d] + 0.5 * (ds[d] / 2^(L + 1)) * RMT[DIM][c][d]
                end
                for k = 1:NDF
...
function coarsen_grid_stream!(
    ...
    @inbounds while index <= n
        ...
        if fl > 0
            if flag[fl] % 1 == 0.0 &&
               index + nc - 1 <= n &&
               _all_same_level(vs_data, index, fl, nc)
                grp = true
                for g = 0:(nc-1)
