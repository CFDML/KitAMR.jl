...
function initialize_vs_data(
    ...
    vs_num = reduce(*, trees_num)
    index = 1;
    midpoint = Matrix{Float64}(undef, vs_num, DIM);
    weight = Vector{Float64}(undef, vs_num)
    vcoords = quadrature.vcoords;
    weights = quadrature.weights
    for i = 1:NP
        for j = 1:NP
            midpoint[index, 1] = vcoords[i];
            midpoint[index, 2] = vcoords[j]
            weight[index] =
                weights[i]*exp(midpoint[index, 1]^2)*weights[j]*exp(midpoint[index, 2]^2)
            index += 1
        end
    end
    ...
    for i = 1:NP
        for j = 1:NP
            for k = 1:NP
                midpoint[index, 1] = vcoords[i];
                midpoint[index, 2] = vcoords[j];
                midpoint[index, 3] = vcoords[k]
                weight[index] =
                    weights[i]*exp(midpoint[index, 1]^2)*weights[j]*exp(
                        midpoint[index, 2]^2,
                    )*weights[k]*exp(midpoint[index, 3]^2)
                index += 1
            end
        end
    end
