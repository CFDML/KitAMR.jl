...
function write_VTK(
    ...
    p4est_geo =
        p4est_geometry_new_connectivity(pointer(PointerWrapper(ps4est).connectivity))
    ...
    GC.@preserve ps4est ppas pas p4est_iterate(ps4est, C_NULL, ppas, cfn, C_NULL, C_NULL)
    ...
    cont =
        p4est_vtk_write_cell_dataf(cont, 1, 1, 1, 0, length(fieldnames), 0, pairs..., cont)
...
function write_VTK(
    ...
    p4est_geo =
        p8est_geometry_new_connectivity(pointer(PointerWrapper(ps4est).connectivity))
    ...
    cont =
        p8est_vtk_write_cell_dataf(cont, 1, 1, 1, 0, length(fieldnames), 0, pairs..., cont)
