...
function solid_static_replace!(forest,which_tree,num_out,out_quads::Ptr{T},num_in,in_quads)::Cvoid where{T}
    GC.@preserve forest which_tree num_out out_quads num_in in_quads begin
        ...
        solid_static_replace!(Val(Int(num_out)),out_quads_wrap,in_quads_wrap,which_tree,ka)
        return nothing
    end
end
function solid_static_replace!(::Val{1}, out_quad, in_quads, which_tree, ka::KA{DIM,NDF}) where{DIM,NDF}# refine replace
                ...
                if solid_flag(ib,midpoint) # Inside solid
                    ...
                    for k in axes(vs_data.df,2)
                        for j in axes(vs_data.df,1)
                            @views vs_data.df[j,k]+=min(abs(vs_data.df[j,k]/(0.5*dot(ps_data.ds,abs.(vs_data.sdf[j,k,:]))+EPS)),1.)*dot(vs_data.sdf[j,k,:],0.5 * ps_data.ds .* RMT[DIM][i])
                        end
                    end
...
function solid_static_replace!(::ChildNum, out_quad, in_quads, which_tree, ka::KA{DIM,NDF}) where{DIM,NDF} # coarsen replace, average or interpolate? Currently interpolation strategy is adopted. If my memory serves me right, problems came out with average most likely due to the iterative balance process.
    trees = ka.kdata.field.trees
    treeid = Int(which_tree) - trees.offset
    datas = trees.data[treeid]
