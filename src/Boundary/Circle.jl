function boundary_flag(
    ...
    for i = 1:4
        flag +=
            norm(midpoint .+ ds .* NMT[2][i] .- boundary.center)>boundary.radius ? 1 : -1 # any neighbor cross boundary?
    end
...
function boundary_flag(
    ...
    for i = 1:6
        flag +=
            norm(midpoint .+ ds .* NMT[3][i] .- boundary.center)>boundary.radius ? 1 : -1 # any neighbor cross boundary?
    end
...
function calc_intersect(f_midpoint, s_midpoint, ::Vector, ::Int, circle::Sphere)
    ...
    x = r_midpoint[dir%3+1];
    y = r_midpoint[(dir+1)%3+1]
    ap = copy(f_midpoint);
    n = copy(f_midpoint)
    rz = sqrt(r^2-x^2-y^2)
    dz =
        abs((rz-r_midpoint[dir])/(s_midpoint[dir]-f_midpoint[dir])) < 1 ?
        rz-r_midpoint[dir] : -rz-r_midpoint[dir]
    ap[dir] = dz+f_midpoint[dir];
    n[dir] = dz+r_midpoint[dir];
    n/=(circle.solid ? r : -r)
    return ap, n
end
...
function solid_box_flag(midpoint, ds, ib::Circle)
    ...
    hyper_rec = HyperRectangle(
        SVector{2,Float64}(ib.center .- ib.radius .- r),
        SVector{2,Float64}(ib.center .+ ib.radius .+ r),
    )
...
function solid_box_flag(midpoint, ds, ib::Sphere)
    r = ib.search_radius
    hyper_rec = HyperRectangle(
        SVector{3,Float64}(ib.center .- ib.radius .- r),
        SVector{3,Float64}(ib.center .+ ib.radius .+ r),
    )
    lower = midpoint-0.5*ds;
    upper = midpoint+0.5*ds
