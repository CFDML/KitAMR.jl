...
function periodic_ghost_cell(midpoint::Vector,ps_data::AbstractGhostPsData{DIM,NDF}) where{DIM,NDF}
    GhostPsData{DIM,NDF}(ps_data.owner_rank,ps_data.quadid,ps_data.bound_enc,
        ps_data.ds,midpoint,ps_data.w,ps_data.sw,ps_data.vs_data
    )
end
function periodic_ghost_cell(midpoints::Vector{Vector{Float64}},ps_datas::Vector{NeighborQuad{DIM,NDF}}) where{DIM,NDF}
