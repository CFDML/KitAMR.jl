...
function any_recursion_test(node::TreeNode, midpoint, ds, lower, upper, tkdt)
    ...
    if overlap_test(lower, upper, table[id]) # kernel intersect test
        if isempty(children(node))
            ...
            lc, rc = children(node)
            any_intersect_recursion_test(lc, midpoint, ds, lower, upper, tkdt) &&
                return true
...
function calc_intersect(f_midpoint, s_midpoint, ds, dir, ib::Triangles)
    ...
    for id in intersect_ids
        ...
        (intersect_point[dir]-s_midpoint[dir])*(intersect_point[dir]-f_midpoint[dir])>0 &&
            continue
        ...
        if in_triangle_test(intersect_point, V, edges)
            ...
            normal =
                dot(normal, f_midpoint-s_midpoint)>0 ? normal/norm(normal) :
                -normal/norm(normal)
...
function solid_box_flag(midpoint, ds, ib::Triangles)
    ...
    hyper_rec = HyperRectangle(
        SVector{3,Float64}(triangle_rec.mins .- r),
        SVector{3,Float64}(triangle_rec.maxes .+ r),
    )
    lower = midpoint-0.5*ds;
    upper = midpoint+0.5*ds
...
function ray_casting_rectangle_test(hyper_rec::HyperRectangle, midpoint, dir, is_pos)
    dir2 = dir%3+1;
    dir3 = (dir+1)%3+1
    if (hyper_rec.mins[dir2]<=midpoint[dir2]<=hyper_rec.maxes[dir2])&&(
