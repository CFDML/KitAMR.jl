...
function save_boundary_result!(
    ...
    kinfo = ka.kinfo;
    ib = kinfo.config.IB[ps_data.bound_enc]
    ...
    for j in axes(ssdf, 2)
        for i in axes(ssdf, 1)
            ...
            ssdf[i, j] =
                min(abs((ib_df[i, j]-eps())/(ssdf[i, j]*dxL+eps())), 1.0)*ssdf[i, j]
        end
    end
...
function solid_cell_flag(
    ...
    for boundary in boundaries
        solid_cell_flag(boundary, midpoint, ds, kinfo, solid_flag(boundary, midpoint)) &&
            return true
    end
    return false
end
...
function initialize_cutted_velocity_cell(
    ...
    du = [
        ...
        kinfo.config.vs_trees_num[i] for i = 1:2
    ]
...
function initialize_cutted_velocity_cell(
    ...
    du = [
        (kinfo.config.quadrature[2*i] - kinfo.config.quadrature[2*i-1]) /
        kinfo.config.vs_trees_num[i] for i = 1:3
    ]
    vertices = zeros(3, 8)
    index = Int[];
    solid_weights = Float64[];
