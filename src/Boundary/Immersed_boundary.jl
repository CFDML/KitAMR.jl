...
function save_boundary_result!(
    ib::AbstractBoundaryType,
    ps_data,
    solid_neighbor::SolidNeighbor{DIM,NDF},
    boundary_results,
    ...
    aux_point = solid_neighbor.aux_point;
    n = solid_neighbor.normal
    faceid = solid_neighbor.faceid
    dir = get_dir(faceid)
    solid_cell = solid_neighbor.solid_cell;
    s_vs_data = solid_cell.vs_data
    ...
    boundary_slope!(
        ...
        vs_data.df,
        s_vs_data.df,
        ib_point[dir]-ps_data.midpoint[dir],
        ps_data.midpoint[dir]-solid_neighbor.midpoint[dir],
        amr,
    )
    @. aux_df = ib_df+ssdf*(aux_point[dir]-ib_point[dir])
    cvc_gas_correction!(aux_df, solid_neighbor)
    aux_prim = get_bc(ib.bc; intersect_point = aux_point, ib);
    aux_prim[1] = 1.0
    ...
    push!(
        boundary_results[ps_data.bound_enc].ps_solutions,
        Boundary_PS_Solution(aux_prim, aux_qf, aux_p),
    )
    if NDF==2
        @suppress write_vs_VTK(
            aux_df,
            vs_data,
            amr,
            dir_path*"/"*string(ps_data.midpoint)*string(n),
            ["h", "b"],
            fieldvalues_fn,
        )
    elseif NDF==1
...
function solid_cell_flag(
    boundaries::Vector{AbstractBoundaryType},
    ...
    for boundary in boundaries
        solid_cell_flag(
            ...
            ds,
            global_data,
            solid_flag(boundary, midpoint),
        ) && return true
    end
    return false
end
function InsideSolid_flag(
    boundary::AbstractBoundaryType,
    midpoint::AbstractVector,
    ds::AbstractVector,
    global_data::Global_Data,
)
    ...
    (
        solid_flag(boundary, midpoint)&&!solid_cell_flag(
            boundary,
            midpoint,
            ds,
            global_data,
    ...
    ) && return true # In solid region and not the ghost cell( a.k.a. solid cell)
    return false
end
function InsideSolid_flag(
    boundaries::Vector{AbstractBoundaryType},
...
function broadcast_boundary_midpoints!(
    boundary_points::Vector{Vector{Vector{Float64}}},
    ::Global_Data{DIM},
) where {DIM}
    ...
    for i in eachindex(boundary_points)
        ...
        for j in eachindex(Numbers)
            if j-1 == MPI.Comm_rank(MPI.COMM_WORLD)
                ...
                for k = 1:Int(length(rbuffer[i][j])/DIM)
...
function IB_flag(
    aux_points::Vector{Vector{Vector{Float64}}},
    midpoint::AbstractVector,
    ds::AbstractVector,
    level::Int8,
    global_data,
    ...
    for i in eachindex(boundaries)
        ...
        for j in eachindex(aux_points[i])
            IB_flag(boundaries[i], aux_points[i][j], midpoint, ds, level, global_data) &&
                return true
        end
    end
    return false
...
function vs_extrapolate!(
    df::AbstractMatrix{Float64},
    sdf::AbstractMatrix{Float64},
    level::AbstractVector{Int8},
    dft::AbstractMatrix{Float64},
    levelt::Vector{Int8},
    dx::Float64,
    ::KitAMR_Data{DIM,NDF},
) where {DIM,NDF}
    ...
    @inbounds for i in axes(dft, 1)
        ...
        elseif levelt[i] < level[j]
            while flag != 1.0
                @. dft[i, :] +=
                    @views (df[j, :]+sdf[j, :]*dx) / 2^(DIM * (level[j] - levelt[i]))
...
function vs_extrapolate!(
    ...
    @inbounds for i in axes(dft, 1)
        ...
        elseif levelt[i] < level[j]
            while flag != 1.0
                @. dft[i, :] +=
                    @views (df[j, :]+sdf[j, :]*dx) / 2^(DIM * (level[j] - levelt[i]))*weight[i]
                flag += 1 / 2^(DIM * (level[j] - levelt[i]))
                j += 1
            end
            flag = 0.0
...
function vs_extrapolate!(
    ...
    @inbounds for i in axes(dft, 1)
        ...
        elseif levelt[i] < level[j]
            while flag != 1.0
                @. dft[i, :] +=
                    @views (df[j, :]+ddf[j, :]) / 2^(DIM * (level[j] - levelt[i]))*weight[i]
                flag += 1 / 2^(DIM * (level[j] - levelt[i]))
                j += 1
            end
            flag = 0.0
...
function update_solid_cell!(
    ::Type{T},
    ps_data::PS_Data{DIM,NDF},
    fluid_cells::Vector,
    amr::KitAMR_Data{DIM,NDF},
) where {DIM,NDF,T<:Union{DVM,CAIDVM,UGKS}}
    vs_data = ps_data.vs_data;
    vs_data.df.=0.0
    weights = Matrix{Float64}(undef, vs_data.vs_num, length(fluid_cells))
    for i in eachindex(fluid_cells)
        l = ps_data.midpoint-fluid_cells[i].midpoint;
        l/=norm(l)
        weights[:, i] .= [max(0.0, dot(u, l)/norm(u))^2 for u in eachrow(vs_data.midpoint)]
    end
    ...
    for i in eachindex(fluid_cells)
        ...
        for j in eachindex(weight_i)
            weight_i[j] =
                weight_sum[j]==0.0 ? 1.0/length(fluid_cells) : weights[j, i]/weight_sum[j]
        end
        fdf = f_vs_data.df;
        fsdf = f_vs_data.sdf;
        dx = ps_data.midpoint-fluid_cells[i].midpoint
        vs_extrapolate!(
            fdf,
            fsdf,
            f_vs_data.level,
...
function update_solid_cell!(
    ...
    fluid_dirs = findall(
        x->!isnothing(x[1])&&!isa(x[1], AbstractInsideSolidData)&&x[1].bound_enc>=0,
...
function initialize_cutted_velocity_cell(
    n::Vector{Float64},
    vs_data::VS_Data{2},
    amr::KitAMR_Data{2,NDF},
) where {NDF}
    any(x->abs(x)<1e-6, n)&&(
        return CuttedVelocityCells(
            Int[],
            copy(vs_data.weight),
    ...
    du = [
        ...
        global_data.config.vs_trees_num[i] for i = 1:2
    ]
    vertices = [zeros(2) for _ = 1:4]
    index = Int[];
    solid_weights = Float64[];
    gas_weights = Float64[]
    for i = 1:vs_data.vs_num
        ...
        if flag
            push!(index, i);
            push!(solid_weights, solid_weight);
            push!(gas_weights, gas_weight)
        end
    end
    N = length(index)
    gas_dfs = zeros(N, NDF);
    solid_dfs = zeros(N, NDF)
...
function initialize_cutted_velocity_cell(
    ...
    du = [
        (global_data.config.quadrature[2*i] - global_data.config.quadrature[2*i-1]) /
        global_data.config.vs_trees_num[i] for i = 1:3
    ]
    vertices = zeros(3, 8)
    index = Int[];
    solid_weights = Float64[];
...
$(TYPEDSIGNATURES)
"""
function initialize_solid_neighbor!(
    ps_data::PS_Data{DIM,NDF},
    amr::KitAMR_Data{DIM,NDF},
) where {DIM,NDF}
    solid_dirs =
        findall(x->!isnothing(x[1])&&x[1].bound_enc<0, ps_data.neighbor.data[1:(2*DIM)])
    vs_data = ps_data.vs_data
    for i in solid_dirs
        ...
        aux_point, normal = calc_intersect(
            ps_data.midpoint,
            solid_cell.midpoint,
            ps_data.ds,
            get_dir(i),
...
function vs_interpolate!(
    f_df::AbstractMatrix,
    f_level::AbstractVector{Int8},
    fx,
    s_df,
    s_level,
    sx,
...
function cvc_density(
    aux_df,
    vn::AbstractVector,
    Θ,
    solid_neighbor::SolidNeighbor,
    MuR::Real,
)
    cvc = solid_neighbor.cvc
...
function cvc_correction!(aux_df, F::AbstractMatrix, solid_neighbor, amr)
    ...
    for i in eachindex(cvc.indices)
        ...
        @views @. aux_df[cvc.indices[i], :] =
            (cvc.gas_weights[i]*cvc.gas_dfs[i, :]+cvc.solid_weights[i]*cvc.solid_dfs[i, :])/(
                cvc.gas_weights[i]+cvc.solid_weights[i]
            )
    end
end
...
function update_solid_neighbor!(
    ::Type{T},
    ps_data::PS_Data{DIM,NDF},
    solid_neighbor::SolidNeighbor{DIM,NDF},
    amr::KitAMR_Data,
) where {DIM,NDF,T<:AbstractFluxType}
    global_data = amr.global_data;
    ...
    solid_neighbor.w =
        calc_w0(vs_data.midpoint, solid_neighbor.vs_data.df, vs_data.weight, global_data)
    solid_neighbor.sw[:, dir] .=
        (solid_neighbor.w-ps_data.w) ./ (solid_neighbor.midpoint[dir]-ps_data.midpoint[dir])
end
function update_solid_neighbor!(
    ps_data::PS_Data{DIM,NDF},
    amr::KitAMR_Data{DIM,NDF},
    ...
    for i in solid_neighbors
        update_solid_neighbor!(
            amr.global_data.config.solver.flux,
            ps_data,
            ps_data.neighbor.data[i][1],
            amr,
        )
