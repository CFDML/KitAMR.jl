...
function adaptive_mesh_refinement!(
    ...
    flag = false
    if amr.global_data.config.solver.PS_DYNAMIC_AMR&&amr.global_data.status.ps_adapt_step >
                                                     ps_interval*converge_ratio
        ps_adaptive_mesh_refinement!(ps4est, amr)
        flag = true;
        amr.global_data.status.ps_adapt_step = 0
    end
    if amr.global_data.config.solver.VS_DYNAMIC_AMR&&amr.global_data.status.vs_adapt_step >
    ...
    if (
        amr.global_data.config.solver.PS_DYNAMIC_AMR||amr.global_data.config.solver.VS_DYNAMIC_AMR
    )&&amr.global_data.status.partition_step>partition_interval*converge_ratio
