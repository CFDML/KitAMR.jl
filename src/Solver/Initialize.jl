...
function initialize_faces!(p4est::Ptr{p4est_t}, ka::KA)
    ...
    GC.@preserve ka AMR_face_iterate(
        p4est;
        user_data = p_data,
...
function initialize_domain_face!(
    ...
    base_quad = unsafe_pointer_to_objref(
        pointer(PointerWrapper(P4estPsData, side.is.full.quad.p.user_data[]).ps_data),
    )
...
function initialize_full_face!(
    ...
    midpoint[direction] -= 0.5*rot*base_quad.ds[direction]
    kinfo = ka.kinfo
    if midpoint[direction] == kinfo.config.geometry[2*direction-1]||midpoint[direction] ==
                                                                    kinfo.config.geometry[2*direction]
...
function initialize_hanging_face!(
    ...
    for i in eachindex(neighbor)
        midpoint[i][direction] =
            base_quad.midpoint[direction] - 0.5 * rot * base_quad.ds[direction]
    end
    kinfo = ka.kinfo
    if midpoint[1][direction] == kinfo.config.geometry[2*direction-1]||midpoint[1][direction] ==
                                                                       kinfo.config.geometry[2*direction]
...
function initialize_back_hanging_face!(
    ...
    for i in ids
        ...
        base_quad = unsafe_pointer_to_objref(
            pointer(PointerWrapper(P4estPsData, qp.p.user_data[]).ps_data),
        )
    ...
    if midpoint[1][direction] == kinfo.config.geometry[2*direction-1]||midpoint[1][direction] ==
        ...
        there_midpoint[direction] =
            midpoint[1][direction] - rot*first(here_data).ds[direction]
        push!(
            faces,
...
function re_init_vs4est!(trees, kinfo)
    for i in eachindex(trees.data)
        for j in eachindex(trees.data[i])
            ...
            ps_data.vs_data.df .= discrete_maxwell(ps_data, kinfo)
        end
    end
end
...
function initialize(
    ...
    prerefine_recursive::Bool = false,
    prerefine_reinit_ic::Bool = true,
)
    kinfo = KInfo(config)
