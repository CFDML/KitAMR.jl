...
function initialize_full_face!(
    ...
    midpoint[direction] -= 0.5*rot*base_quad.ds[direction]
    global_data = amr.global_data
    if midpoint[direction] == global_data.config.geometry[2*direction-1]||midpoint[direction] ==
...
function initialize_hanging_face!(
    ...
    global_data = amr.global_data
    if midpoint[1][direction] == global_data.config.geometry[2*direction-1]||midpoint[1][direction] ==
                                                                             global_data.config.geometry[2*direction]
...
function init_solid_midpoints_kernel(ip, data, dp)
    ...
    for i in eachindex(boundaries)
        ...
        solid_cell_flag(boundaries[i], midpoint, ds, global_data, inside)&&push!(
            solid_midpoints[i],
            midpoint,
        )
    end
