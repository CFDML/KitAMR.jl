...
function ps_refine_flag(
    ...
    global_data.config.user_defined.static_ps_refine_flag(
        ps_data.midpoint,
        ps_data.ds,
        global_data,
        qp.level[],
    ...
    if rgrad >
       2.0^((qp.level[] - global_data.config.solver.AMR_DYNAMIC_PS_MAXLEVEL)) *
       ADAPT_COEFFI_PS # 2 for second-order scheme
        flag = Cint(1)
    else
        flag = Cint(0)
    end
...
function ps_coarsen_flag(
    ...
    for i = 1:(2^DIM)
        ps_data = ps_datas[i]
        (ps_data.bound_enc!=0||domain_flag(global_data, ps_data.midpoint, ps_data.ds)) &&
            return Cint(0)
        global_data.config.user_defined.static_ps_refine_flag(
            ps_data.midpoint,
            ps_data.ds,
        ...
        rgrad = maximum(agrad ./ gradmax)
        if rgrad >
           2.0^((levels[i]-1 - global_data.config.solver.AMR_DYNAMIC_PS_MAXLEVEL)) *
           ADAPT_COEFFI_PS # 2 for second-order scheme
            return Cint(0)
        end
        agrad.=0.0
