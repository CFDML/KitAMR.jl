...
@inline function velocity_slope(
    sw::AbstractMatrix,
    prim::AbstractVector,
    component::Integer,
    dir::Integer,
...
@inline function velocity_scale(prim::AbstractVector)
    speed2 = 0.0
    @inbounds for i = 2:(length(prim)-1)
...
@inline function vorticity_amplitude_ok(
    left::Real,
    center::Real,
    right::Real,
    prim::AbstractVector,
...
$(TYPEDSIGNATURES)
"""
function ps_refine_flag(ps_data::PsData{DIM}, level::Int8, ka::KA{DIM}) where {DIM}
    kinfo = ka.kinfo
    ...
    dflag =
        kinfo.config.user_defined.dynamic_ps_refine_flag==null_udf ? true :
        kinfo.config.user_defined.dynamic_ps_refine_flag(ps_data, level, ka)
    !dflag&&return Cint(0)
...
function ps_coarsen_flag(
    ...
    for i = 1:(2^DIM)
        ps_data = ps_datas[i]
        (ps_data.bound_enc!=0||domain_flag(kinfo, ps_data.midpoint, ps_data.ds)) &&
            return Cint(0)
        kinfo.config.user_defined.static_ps_refine_flag(
            ps_data.midpoint,
            ps_data.ds,
...
function update_Lohner_inner_ps!(
    ...
    use_vorticity =
        vorticity_amplitude_ok(omegaL, omega, omegaR, ps_data.prim, max(dsL, dsR))

    eps_l = 0.2*ps_data.ds[dir]
    @inbounds for j in eachindex(ws_swL)
        if j == 2
            ps_data.lohner[j, dir] =
                use_vorticity ? lohner_value(omegaL, omega, omegaR, dsL, dsR, eps_l) : 0.0
        else
            ps_data.lohner[j, dir] =
