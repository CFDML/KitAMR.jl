...
function vs_merge!(sdf::AbstractMatrix,sdf_n::AbstractMatrix,level::Vector,level_n::Vector,::KA{DIM}) where{DIM}
    j = 1
    flag = 0.0
    for i in axes(sdf,1)
        if level[i] == level_n[j]
...
#     return false
# end
function boundary_flag(boundary::Domain,midpoint::AbstractVector,ds::AbstractVector,kinfo::KInfo) # Domain boundary flag
...
"""
function ps_replace!(::ChildNum, out_quad, in_quads, which_tree, ka::KA{DIM,NDF}) where{DIM,NDF} # coarsen replace, average or interpolate? Currently interpolation strategy is adopted. If my memory serves me right, problems came out with average most likely due to the iterative balance process.
    trees = ka.kdata.field.trees
    treeid = Int(which_tree) - trees.offset
    datas = trees.data[treeid]
