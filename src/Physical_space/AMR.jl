...
function boundary_flag(
    ...
    abs(midpoint[index] - global_data.config.geometry[boundary.id]) < ds[index] &&
        return true
end
...
function ps_refine_flag(::InsideSolidData, ::KitAMR_Data, ::PW_pxest_quadrant_t)
    Cint(0)
end
...
function ps_replace!(
    ...
    for i = 1:(2^DIM)
        ...
        if flag
            ...
            for j = 1:DIM
                @. vs_data.df +=
                    0.5 * ps_data.ds[j] * RMT[DIM][i][j] * @view(vs_data.sdf[:, :, j])
            end
        ...
        for i = 1:(2^DIM)
            ...
            (
                global_data.config.user_defined.static_ps_refine_flag(
                    ...
                    global_data,
                    qp.level[]-1,
                )&&!solid_flag(midpoint, global_data) ||
