...
function calc_domain_flux(
    ...
    @inbounds @views dx = [
        midpoint[j]-here_mid[i, j]*Δt-ps_data.midpoint[j] for
        i in axes(here_mid, 1), j in axes(here_mid, 2)
    ]
    @inbounds @views df = [
        here_df[i, j]+dot(dx[i, :], here_sdf[i, j, :]) for
    ...
    @inbounds there_micro =
        [there_df[i, j]*there_vn[i] for i in axes(there_df, 1), j in axes(there_df, 2)]
    fw =
        micro_to_macro(here_micro, here_mid, here_weight, vs_data)+micro_to_macro(
            there_micro,
            there_mid,
            there_weight,
            vs_data,
...
function calc_domain_flux(
    ...
    @inbounds @views begin
        ...
        there_df =
            vs_data.df[nheavi, :]+(there_ps_mid[direction]-here_ps_mid[direction])*there_sdf[
                :,
                :,
                direction,
            ]
        ...
        dx = [
            midpoint[j]-here_mid[i, j]*Δt-here_ps_mid[j] for
            i in axes(here_mid, 1), j in axes(here_mid, 2)
        ]
        ndx = [
            midpoint[j]-there_mid[i, j]*Δt-there_ps_mid[j] for
        ...
        ndf = [
            there_df[i, j]+dot(ndx[i, :], there_sdf[i, j, :]) for
...
function calc_flux(
...
) where {DIM,NDF} # without face area and Δt
    _, _, midpoint, here_data, there_data = unpack(flux_data)
    Δt = amr.global_data.status.Δt
    here_mid = here_vs.midpoint;
    there_mid = there_vs.midpoint
    here_vn = here_vs.vn;
    there_vn = there_vs.vn
    here_df = here_vs.df;
    ...
    @inbounds @views begin
        ...
        if there_data.bound_enc<0
            here_micro = [
                (here_df[i, j]+dot(dx[i, :], here_sdf[i, j, :]))*here_vn[i] for
                i in axes(here_df, 1), j in axes(here_df, 2)
            ]
            there_micro = [
                (there_df[i, j]+dot(ndx[i, :], there_sdf[i, j, :]))*there_vn[i] for
                i in axes(there_df, 1), j in axes(there_df, 2)
            ]
        else
            here_micro = positivity_preserving_reconstruct(
                here_df,
                here_sdf,
                here_data.ds,
                dx,
            ...
            there_micro = positivity_preserving_reconstruct(
                there_df,
                there_sdf,
                there_data.ds,
                ndx,
    ...
    here_weight = here_vs.weight;
    there_weight = there_vs.weight
    fw =
        micro_to_macro(here_micro, here_mid, here_weight, here_data.vs_data)+micro_to_macro(
            there_micro,
            there_mid,
            there_weight,
            here_data.vs_data,
...
function positivity_preserving_reconstruct(here_df, here_sdf, here_ds, dx, vn) # positivity preserving reconstruct (10.1016/j.jcp.2009.12.030)
    @views begin
        micro = [
            (
                here_df[i, j]+min(
                    abs(here_df[i, j]/(0.5*dot(here_ds, abs.(here_sdf[i, j, :]))+EPS)),
                    1.0,
                )*dot(dx[i, :], here_sdf[i, j, :])
            )*vn[i] for i in axes(here_df, 1), j in axes(here_df, 2)
        ]
    end
    return micro
end
