function calc_domain_flux(::Type{DVM},here_vs::FaceVsData,face::DomainFace{DIM,NDF,Maxwellian},ka::KA) where{DIM,NDF}
...
function calc_domain_flux(::Type{DVM},here_vs::FaceVsData,face::DomainFace{DIM,NDF,SuperSonicInflow},ka::KA) where{DIM,NDF}
    _,direction,midpoint,_,ps_data = unpack(face)
    heavi,_,here_mid,here_vn,here_df,here_sdf = unpack(here_vs)
    Δt = ka.kinfo.status.Δt
    vs_data = ps_data.vs_data
...
function calc_domain_flux(::Type{DVM},here_vs::FaceVsData,face::DomainFace{DIM,NDF,UniformOutflow},ka::KA) where{DIM,NDF}
...
function calc_domain_flux(::Type{DVM},here_vs::FaceVsData,face::DomainFace{2,NDF,InterpolatedOutflow},ka::KA) where{NDF}
    _,direction,midpoint,_,ps_data = unpack(face)
    heavi,_,here_mid,here_vn,here_df,here_sdf = unpack(here_vs)
    Δt = ka.kinfo.status.Δt
...
function calc_flux(::Type{DVM},here_vs,there_vs,flux_data::Union{FullFace,FluxData},ka::KA{DIM,NDF}) where{DIM,NDF} # without face area and Δt
