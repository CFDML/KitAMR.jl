function calc_domain_flux(
    ::Type{UGKS},
    here_vs::FaceVsData,
    face::DomainFace{2,2,SuperSonicInflow},
    ka::KA,
)
...
function calc_domain_flux(
    ::Type{UGKS},
    here_vs::FaceVsData,
    face::DomainFace{2,2,UniformOutflow},
    ka::KA,
)
    rot, direction, midpoint, _, here_data = unpack(face)
    ...
    ndf = there_df
    Here_vs =
        FaceVsData{2,2}(heavi, here_weight, here_mid, here_vn, df, zeros(size(here_sdf)))
    There_vs = FaceVsData{2,2}(
        nheavi,
        there_weight,
        there_mid,
        there_vn,
        ndf,
        zeros(size(there_sdf)),
    )
    w0 = calc_w0(Here_vs, There_vs)
    prim0 = get_prim(w0, kinfo)
    qf0 = calc_qf(Here_vs, There_vs, prim0)
...
function calc_domain_flux(
    ::Type{UGKS},
    here_vs::FaceVsData,
    face::DomainFace{2,2,InterpolatedOutflow},
    ka::KA,
)
    rot, direction, midpoint, _, here_data = unpack(face)
    heavi, here_weight, here_mid, here_vn, here_df, here_sdf = unpack(here_vs)
    ...
    there_weight = @views vs_data.weight[nheavi]
    there_vn = @views there_mid[:, direction]
    there_df = @views vs_data.df[nheavi, :]+2.0*vs_data.sdf[nheavi, :, direction]*(
        midpoint[direction]-here_ps_mid[direction]
    )
    there_sdf = @views vs_data.sdf[nheavi, :, :]
    ...
    @inbounds @views begin
        df = [
            here_df[i, j]+dot(dx, here_sdf[i, j, :]) for
            i in axes(here_df, 1), j in axes(here_df, 2)
        ]
        ndf = [
            there_df[i, j]+dot(ndx, there_sdf[i, j, :]) for
            i in axes(there_df, 1), j in axes(there_df, 2)
        ]
    end
...
function calc_flux_f0(
    hvd::FaceVsData{2,2},
    tvd,
    here_F::AbstractMatrix,
    there_F,
    Mt,
    direction::Int,
)
...
function calc_flux(
    ...
    here_vs,
    there_vs,
    flux_data::Union{FullFace,FluxData},
    ka::KA{2,2},
) # without face area
    ...
    Δt = kinfo.status.Δt
    here_df = here_vs.df;
    there_df = there_vs.df
    here_sdf = here_vs.sdf;
    there_sdf = there_vs.sdf
    here_ps_mid = here_data.midpoint;
    there_ps_mid = there_data.midpoint
    dx = midpoint-here_ps_mid
    ...
    Here_vs = FaceVsData(here_vs, df);
    There_vs = FaceVsData(there_vs, ndf)
    w0 = calc_w0(Here_vs, There_vs)
    prim0 = get_prim(w0, kinfo)
    if 1/prim0[end]>1e-6
        qf0 = calc_qf(Here_vs, There_vs, prim0)
        ...
        here_micro *= kinfo.status.Δt;
        there_micro *= kinfo.status.Δt
        fw *= kinfo.status.Δt
    end
    return fw, [here_micro, there_micro]
end
