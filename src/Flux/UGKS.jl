function calc_domain_flux(::Type{UGKS},here_vs::FaceVsData,face::DomainFace{2,2,SuperSonicInflow},ka::KA)
    ...
    @inbounds for i in axes(ndf,1)
        @views ndf[i,:] .= discrete_maxwell(there_mid[i,:],bc,kinfo)
    end
    Here_vs = FaceVsData(here_vs,df);There_vs = FaceVsData{2,2}(nheavi,there_weight,there_mid,there_vn,ndf,there_sdf)
    w0 = calc_w0(Here_vs,There_vs)
    prim0 = get_prim(w0, kinfo)
    qf0 = calc_qf(Here_vs,There_vs,prim0)
    aL, aR = calc_a(w0, prim0, here_data.w, domain_w, here_data.ds[direction], here_data.ds[direction], kinfo, rot)
    Mu, Mv, Mξ, Mu_L, Mu_R = moment_u(prim0, kinfo, rot, direction)
    A = calc_A(prim0, aL, aR, Mu, Mv, Mξ, Mu_L, Mu_R, kinfo, direction)
    τ0 = get_τ(prim0, gas.μᵣ, gas.ω)
    Mt = time_int(τ0, Δt)
    fw = calc_flux_g0_2D2F(prim0, Mt, Mu, Mv, Mξ, Mu_L, Mu_R, aL, aR, A, direction)
    here_F = discrete_maxwell(Here_vs.midpoint, prim0, kinfo)
    there_F = discrete_maxwell(There_vs.midpoint, prim0, kinfo)
    here_F⁺ = shakhov_part(Here_vs.midpoint, here_F, prim0, qf0, kinfo)
    there_F⁺ = shakhov_part(There_vs.midpoint, there_F, prim0, qf0, kinfo)
...
function calc_domain_flux(::Type{UGKS},here_vs::FaceVsData,face::DomainFace{2,2,UniformOutflow},ka::KA)
    rot,direction,midpoint,_,here_data = unpack(face)
    ...
    df = here_df
    ndf = there_df
    Here_vs = FaceVsData{2,2}(heavi,here_weight,here_mid,here_vn,df,zeros(size(here_sdf)))
    There_vs = FaceVsData{2,2}(nheavi,there_weight,there_mid,there_vn,ndf,zeros(size(there_sdf)))
    w0 = calc_w0(Here_vs,There_vs)
    prim0 = get_prim(w0, kinfo)
    qf0 = calc_qf(Here_vs,There_vs,prim0)
    aL, aR = calc_a(w0, prim0, here_data.w, w0, here_data.ds[direction], here_data.ds[direction], kinfo, rot)
    Mu, Mv, Mξ, Mu_L, Mu_R = moment_u(prim0, kinfo, rot, direction)
    A = calc_A(prim0, aL, aR, Mu, Mv, Mξ, Mu_L, Mu_R, kinfo, direction)
    τ0 = get_τ(prim0, gas.μᵣ, gas.ω)
    Mt = time_int(τ0, Δt)
...
function calc_domain_flux(::Type{UGKS},here_vs::FaceVsData,face::DomainFace{2,2,InterpolatedOutflow},ka::KA)
    rot,direction,midpoint,_,here_data = unpack(face)
    heavi,here_weight,here_mid,here_vn,here_df,here_sdf = unpack(here_vs)
    kinfo = ka.kinfo
    gas = kinfo.config.gas
    ...
    Here_vs = FaceVsData{2,2}(heavi,here_weight,here_mid,here_vn,df,here_sdf)
    There_vs = FaceVsData{2,2}(nheavi,there_weight,there_mid,there_vn,ndf,there_sdf)
    w0 = calc_w0(Here_vs,There_vs)
    prim0 = get_prim(w0, kinfo)
    qf0 = calc_qf(Here_vs,There_vs,prim0)
    aL, aR = calc_a(w0, prim0, here_data.w, 2.0*w0-here_data.w, here_data.ds[direction], here_data.ds[direction], kinfo, rot)
    Mu, Mv, Mξ, Mu_L, Mu_R = moment_u(prim0, kinfo, rot, direction)
    A = calc_A(prim0, aL, aR, Mu, Mv, Mξ, Mu_L, Mu_R, kinfo, direction)
    τ0 = get_τ(prim0, gas.μᵣ, gas.ω)
    Mt = time_int(τ0, Δt)
...
function calc_w0(here_vs::FaceVsData{2,2},there_vs::FaceVsData)
...
function calc_w0(here_vs::FaceVsData{3,1},there_vs::FaceVsData)
...
function calc_flux_f0(hvd::FaceVsData{2,2},tvd,here_F::AbstractMatrix,there_F,Mt,direction::Int)
...
function calc_flux(::Type{UGKS},here_vs,there_vs,flux_data::Union{FullFace,FluxData},ka::KA{2,2}) # without face area
    rot,direction,midpoint,here_data,there_data = unpack(flux_data)
    kinfo = ka.kinfo
    gas = kinfo.config.gas;Δt = kinfo.status.Δt
    ...
    Here_vs = FaceVsData(here_vs,df);There_vs = FaceVsData(there_vs,ndf)
    w0 = calc_w0(Here_vs,There_vs)
    prim0 = get_prim(w0, kinfo)
    if 1/prim0[end]>1e-6
        qf0 = calc_qf(Here_vs,There_vs,prim0)
        ...
        aL, aR = calc_a(w0, prim0, wL, wR, here_data.ds[direction], there_data.ds[direction], kinfo, rot)
        Mu, Mv, Mξ, Mu_L, Mu_R = moment_u(prim0, kinfo, rot, direction)
        A = calc_A(prim0, aL, aR, Mu, Mv, Mξ, Mu_L, Mu_R, kinfo, direction)
        τ0 = get_τ(prim0, gas.μᵣ, gas.ω)
        Mt = time_int(τ0, Δt)
        ...
        fw, microflux = calc_flux(CAIDVM,here_vs,there_vs,flux_data,ka)
        here_micro,there_micro = microflux
        here_micro *= kinfo.status.Δt;there_micro *= kinfo.status.Δt
        fw *= kinfo.status.Δt
    end
    return fw,[here_micro,there_micro]
end
