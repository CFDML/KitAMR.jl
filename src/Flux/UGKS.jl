function calc_domain_flux(
    ...
    Here_vs = Face_VS_Data(here_vs, df);
...
function calc_domain_flux(
    ...
    ndf = there_df
    Here_vs =
        Face_VS_Data{2,2}(heavi, here_weight, here_mid, here_vn, df, zeros(size(here_sdf)))
    There_vs = Face_VS_Data{2,2}(
        nheavi,
        there_weight,
        there_mid,
        there_vn,
...
function calc_flux(
    ...
    Here_vs = Face_VS_Data(here_vs, df);
    There_vs = Face_VS_Data(there_vs, ndf)
    w0 = calc_w0(Here_vs, There_vs)
    prim0 = get_prim(w0, global_data)
    if 1/prim0[end]>1e-6
        qf0 = calc_qf(Here_vs, There_vs, prim0)
        ...
        here_micro *= global_data.status.Δt;
        there_micro *= global_data.status.Δt
