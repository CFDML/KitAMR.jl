...
function partition!(p4est::Ptr{p4est_t}, weight::Function = partition_weight)
    ...
    p4est_partition(
        p4est,
        0,
        @cfunction($weight, Cint, (Ptr{p4est_t}, p4est_topidx_t, Ptr{p4est_quadrant_t}))
    )
...
function partition!(p4est::Ptr{p8est_t}, weight::Function = partition_weight)
    partition!(
        p4est,
        @cfunction($weight, Cint, (Ptr{p8est_t}, p4est_topidx_t, Ptr{p8est_quadrant_t}))
    )
end
    ...
    for i in eachindex(trees)
        for j in eachindex(trees[i])
            ...
            if index > send_nums[send_index]
                push!(
                    s_datas,
                    Transfer_Data{DIM,NDF}(encs, ws, vs_levels, vs_midpoints, vs_df),
                )
...
function transfer_wrap(
    sends::Vector,
    send_nums::Vector,
    amr::KitAMR_Data{DIM,NDF},
) where {DIM,NDF}
    isempty(sends) &&
        return Vector{Vector{Int}}(undef, 0), Vector{Transfer_Data{DIM,NDF}}(undef, 0)
