    ...
    GC.@preserve ps4est global_data begin
        ...
        for i = 1:gp.mirrors.elem_count[]
            ...
            p = Ptr{Cdouble}(
                sc_malloc(
                    P4est.package_id(),
                    (3 * DIM + 4 + NDF * vs_num) * sizeof(Cdouble),
                ),
            )
    ...
    GC.@preserve ps4est global_data begin
        ...
        for i = 1:gp.mirrors.elem_count[]
            ...
            p = Ptr{Cdouble}(
                sc_malloc(
                    P4est.package_id(),
                    (vs_num * DIM * NDF+DIM*(DIM+2)) * sizeof(Cdouble),
                ),
            )
...
function get_mirror_structure(ps4est, global_data::Global_Data{DIM,NDF}) where {DIM,NDF}
    GC.@preserve ps4est global_data begin
        ...
        for i = 1:gp.mirrors.elem_count[]
            ...
            ps_data = unsafe_pointer_to_objref(pointer(dp.ps_data))
            p = Ptr{Cdouble}(
                sc_malloc(P4est.package_id(), vs_num * (DIM + 2) * sizeof(Cdouble)),
            )
...
function ghost_data_alloc(N::Int, ghost::P_pxest_ghost_t)
    Ptr{Cdouble}(
        sc_malloc(
            P4est.package_id(),
            sizeof(Cdouble) * N * PointerWrapper(ghost).ghosts.elem_count[],
        ),
    )
end
...
function initialize_ghost_exchange(
    ps4est,
...
function initialize_ghost_wrap(
    ...
    ghost_wrap = Array{AbstractGhostPsData}(
        undef,
        PointerWrapper(global_data.forest.ghost).ghosts.elem_count[],
    )
    global_vs_num = global_data.status.max_vs_num
    for i in eachindex(ghost_wrap)
        pq = iPointerWrapper(
            PointerWrapper(global_data.forest.ghost).ghosts,
            p4est_quadrant_t,
            i - 1,
        )
        ...
        sw = Base.unsafe_wrap(
            Matrix{Cdouble},
            p + vs_num * NDF * DIM * sizeof(Cdouble),
            (DIM+2, DIM),
        )
        p = ghost_data_ptr(
        ...
        ghost_wrap[i] = Ghost_PS_Data{DIM,NDF}(
            owner_rank,
            quadid,
            bound_enc,
            ds,
