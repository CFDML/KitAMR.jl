...
function pw_mirror_quadrant(
    pp::PointerWrapper{p4est_t},
    gp::PointerWrapper{p4est_ghost_t},
    i::Integer,
)
...
function pw_mirror_quadrant(
    pp::PointerWrapper{p8est_t},
    gp::PointerWrapper{p8est_ghost_t},
    i::Integer,
)
...
function get_mirror_slope_inner!(
    ...
    vs_temp[(vs_num*NDF*DIM+1):(vs_num*NDF*DIM+DIM*(DIM+2))] .=
        reshape(ps_data.sw, DIM*(DIM+2))
end
function get_mirror_slope_inner!(::InsideSolidData, vs_temp::AbstractVector)
    vs_temp[1] = EPS
end
function get_mirror_structure_inner!(
    ps_data::PsData{DIM},
    weight_temp,
    level_temp,
    midpoint_temp,
...
function ghost_data_alloc(N::Int, ghost::P_pxest_ghost_t)
    Ptr{Float64}(
        sc_malloc(
            P4est.package_id(),
            sizeof(Float64) * N * PointerWrapper(ghost).ghosts.elem_count[],
        ),
    )
end
...
function _ghost_comm_arrays(ghost::P_pxest_ghost_t)
    ...
    proc_offsets =
        Base.unsafe_wrap(Vector{Int32}, pointer(gp.proc_offsets), mpisize + 1, own = false)
    mpo = Base.unsafe_wrap(
        Vector{Int32},
        pointer(gp.mirror_proc_offsets),
        mpisize + 1,
        own = false,
    )
    n_mpm = Int(mpo[end])
    mpm =
        ...
        Base.unsafe_wrap(
            Vector{Int32},
            pointer(gp.mirror_proc_mirrors),
            n_mpm,
            own = false,
        ) : Int32[]
...
function _mpi_exchange_varsize!(
    ...
    @inbounds for recv_id = 1:n_recvs
        ...
        recv_count == length(recv_buf) || error(
            "Direct MPI ghost exchange size mismatch on rank ",
            myrank,
            ": expected ",
            length(recv_buf),
...
function _is_compact_recv_layout(
    ids::Vector{Int},
    ghost_offsets::Vector{Int},
    ghost_elem_sizes::Vector{Int},
)
...
function amr_exchange_varsize(
    ...
    ghost_buffer, ghost_offsets = amr_exchange_varsize(
        ghost,
        mirror_data_bufs,
        mirror_elem_sizes,
        ghost_elem_sizes,
...
function initialize_ghost_wrap(
    kinfo::KInfo{DIM,NDF},
    ...
    for i in eachindex(ghost_wrap)
        ...
        sw = Base.unsafe_wrap(
            Matrix{Float64},
            p_slope + vs_num * NDF * DIM * sizeof(Float64),
            (DIM + 2, DIM),
        )
        ...
        level =
            Base.unsafe_wrap(Vector{Float64}, p_struct + vs_num * sizeof(Float64), vs_num)
        midpoint_vs = Base.unsafe_wrap(
            Matrix{Float64},
            p_struct + 2 * vs_num * sizeof(Float64),
            (vs_num, DIM),
        )

        vs_data =
            GhostVsData{DIM,NDF}(vs_num, Int8.(round.(level)), weight, midpoint_vs, df, sdf)
        ghost_wrap[i] = GhostPsData{DIM,NDF}(
            owner_rank,
            quadid,
            bound_enc,
            ds,
...
function data_exchange!(p4est::P_pxest_t, ka::KA{DIM,NDF}) where {DIM,NDF}
    ...
    amr_exchange_varsize!(
        ...
        gb_buf.ghost_datas,
        gi.ghost_data_offsets,
        gi.ghost_data_szs,
        gb_buf.mirror_data_bufs,
        gi.mirror_data_szs,
        51,
    )
end
...
function slope_exchange!(p4est::P_pxest_t, ka::KA{DIM,NDF}) where {DIM,NDF}
    ...
    _native_exchange_varsize!(
        ...
        ka.kinfo.forest.ghost,
        gb_buf.ghost_slopes,
        gi.ghost_slope_offsets,
        gi.ghost_slope_szs,
        gb_buf.mirror_slope_bufs,
        gi.mirror_slope_szs,
    )
end
