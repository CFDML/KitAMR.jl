...
function shock_wave_region(midpoint, ds, global_data, level)
    if midpoint[1]>-5.0&&midpoint[1]<5.0&&midpoint[2]<5.0&&midpoint[2]>-5.0&&√sum(
        midpoint .^ 2,
    )>1.0&&level<4
        return true
    end
...
function cylinder_buffer_IC(midpoint::Vector{Float64})
    ...
    if r>R+l
        ...
        return [1.0, (r-R)*Ma*√(5/6), 0.0, Tw-(r-R)/l*(Tw-1.0)]
    end
end
function vs_output_flag(; ps_data, kwargs...)
    ...
    tps[1] = [-1.4589554070669333, 0.05709202822871219];
    tps[2] = [-1.3786385660534994, 0.05709202822871219]
    tps[3] = [1.7311, 3.3467];
    tps[4] = [-0.9037743267386753, 0.45412464859673063]
