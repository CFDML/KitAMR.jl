using KitAMR,MPI
MPI.Init()
# config = KitAMR.read_config("configure_convergence_32_v28.txt")
...
#     run(`mv p ./test_convergence_vs/p28`)
# end
# MPI.Barrier(MPI.COMM_WORLD)


# config = KitAMR.read_config("configure_convergence_32_v50.txt")
# ps4est,amr = KitAMR.init(config);
# max_sim_time = 20.
# nt = max_sim_time/amr.global_data.status.Δt+1.0 |> floor |> Int
# for i in 1:nt
...
# KitAMR.finalize!(ps4est,amr)
# if MPI.Comm_rank(MPI.COMM_WORLD)==0
#     run(`mv p ./test_convergence_vs/p50`)
# end
# MPI.Barrier(MPI.COMM_WORLD)

config = KitAMR.read_config("configure_convergence_32_v100.txt")
ps4est,amr = KitAMR.init(config);
max_sim_time = 20.
nt = max_sim_time/amr.global_data.status.Δt+1.0 |> floor |> Int
for i in 1:nt
...
KitAMR.finalize!(ps4est,amr)
if MPI.Comm_rank(MPI.COMM_WORLD)==0
    run(`mv p ./test_convergence_vs/p100`)
end
MPI.Finalize()
