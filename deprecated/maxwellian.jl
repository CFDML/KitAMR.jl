function WLS_coeffi(
    interp_point::AbstractVector{T},
    aux_point::AbstractVector{T},
    points::AbstractVector{Vector{T}},
    R::T,
) where {T} # interp_point: the point to be interpolated (image_point or aux_point); R: search radius
    # w_m is calculated according to the distance to the aux_point
    N = length(points);
    DIM = length(interp_point)
...
function update_solid_cell!(amr::KitAMR_Data)
    ...
    for i in eachindex(boundary.solid_cells)
        @inbounds update_solid_cell!(
            global_data.config.IB[i],
            boundary.solid_cells[i],
            boundary.aux_points[i],
            boundary.IB_cells[i],
            global_data,
        )
    end
...
function vs_projection!(
    vs_data::AbstractVsData{DIM,NDF},
    vs_data_n::AbstractVsData,
    temp::AbstractMatrix,
) where {DIM,NDF}
    j = 1
    flag = 0.0
...
function make_coeffi!(
    coeffi::AbstractVector,
    sigma::Real,
    interp_point::AbstractVector{T},
    Vpoints::AbstractVector{Vector{T}},
) where {T}
...
function make_bilinear_coeffi_2D(image_point::AbstractVector)
    return [image_point[1], image_point[2], image_point[1]*image_point[2], 1.0]
end
function make_bilinear_coeffi_3D(image_point::AbstractVector)
    return [
        ...
        image_point[1]*image_point[3],
        image_point[2]*image_point[3],
        1.0,
    ]
end
...
function update_solid_cell!(
    ...
    for i in eachindex(solidcells.ps_datas)
        ...
        angle = abs(atan(n[2]/n[1]))
        ratio =
            abs(ps_data.prim[end]-IB_cells.IB_nodes[i][end].prim[end])/circle.search_radius/global_data.status.gradmax[end]
        if ratio<10
            # if angle>35/180*π&&angle<55/180*π
            #     temp_id = upwind_template(IB_cells.templates[i],IB_cells.IB_nodes[i],aux_point,IB_cells.IB_nodes[i][1].prim)
            # else
        ...
        points = [IB_cells.IB_nodes[i][temp_id[j]].midpoint for j = 1:4]
        A = bilinear_coeffi_2D(points...) ./ dA
        # if rank(A)<4
        #     @show A
        #     deleteat!(IB_cells.templates[i],id)
        ...
        for j = 2:4
            vs_projection!(
                s_vs_data,
                IB_cells.IB_nodes[i][temp_id[j]].vs_data,
                aux_vs_temp[j],
            )
            # @assert !any(x->isnan(x),aux_vs_temp[j]) `projection error!`,aux_vs_temp[j],IB_cells.IB_nodes[i][j].vs_data.df
        end
        ...
        for j in axes(aux_df, 1)
            if Θ[j]==1.0
                ...
                # aux_df[j,:] .= df.+shakhov_part(@view(s_vs_data.midpoint[j,:]),df,aux_prim,aux_qf,global_data)
                aux_df[j, :] .=
                    discrete_maxwell(@view(s_vs_data.midpoint[j, :]), aux_prim, global_data)
                # @assert !any(x->isnan(x),@views aux_df[j,:]) `maxwell error!`,aux_prim
            end
        end
        @. s_vs_data.df = aux_df+(aux_df-ip_df)/dxL*dxR
...
function update_solid_cell!(
    ...
    for i in eachindex(solidcells.ps_datas)
        ...
        vn = [dot(@view(s_vs_data.midpoint[j, :]), n) for j in axes(s_vs_data.midpoint, 1)]
        Θ = heaviside.(vn)
        points = [IB_cells.IB_nodes[i][j].midpoint for j = 1:7]
        Ainv = inv(bilinear_coeffi_3D(points...))
        b = Vector{Float64}(undef, 7);
        ip_coeffi = make_bilinear_coeffi_3D(image_point)
        @inbounds for j in axes(ip_df, 1)
            for l = 1:NDF
                for k = 1:7
                    b[k] = aux_vs_temp[k][j, l]
...
function project_solid_cell_slope!(
    vs_data::AbstractVsData{DIM,NDF},
    vs_data_n::VS_Data{DIM,NDF},
    DIR::Integer,
) where {DIM,NDF}
    j = 1
    flag = 0.0
...
function calc_solid_cell_slope!(
    svdata::AbstractVsData{DIM,NDF},
    fvdata::VS_Data{DIM,NDF},
    smid::Vector{Float64},
    fmid::Vector{Float64},
    direction::Integer,
) where {DIM,NDF}
    ...
    @inbounds for i = 1:svdata.vs_num
        if level[i] == level_n[j]
