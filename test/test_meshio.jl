        ...
        if isempty(children(node))
            ...
            for i in ids
                if overlap_test(lower,upper,recs[i])
                    h = 0.5*ds;V0 = mesh.position[mesh.faces[i]][1]
...
lower = midpoint-0.5ds;upper = midpoint+0.5ds
...
mesh = tkdt.mesh
h = 0.5*ds;V = mesh.position[mesh.faces[tri_id]]
a = [
        SVector{3,Float64}([1,0,0]),
        SVector{3,Float64}([0,1,0]),
        SVector{3,Float64}([0,0,1])
...
midpoint =  [0.25, 0.7999984, 0.0]; ds = ones(3)*0.25
intersect_ids = all_intersect_test(midpoint,ds,tkdt)
...
function ray_casting_recursion_test!(node::TreeNode,midpoint,dir::Int,is_pos::Bool,tkdt::TriangleKDT,intersect_ids)
    table = tkdt.table;mesh = tkdt.mesh;edges = tkdt.triangle_edges;recs = tkdt.triangle_recs
    id = node.index
    hyper_rec = table[id]
